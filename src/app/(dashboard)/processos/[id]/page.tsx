...
function EspeciesView({ rows }: { rows: EspecieStat[] }) {
  if (!rows.length) return <EmptyState>Sem dados por espécie.</EmptyState>;
  return (
    <ul className="space-y-2">
      {rows.map((e) => (
...
function GargalosView({ g }: { g: RecursoStats['gargalos'] }) {
  ...
  if (empty) return <EmptyState>Nenhum gargalo evidente — sem espécies/bancos com baixa taxa de êxito nem grandes filas de espera.</EmptyState>;
...
function RecursosInsightsPanel() {
  ...
  if (isLoading) return <Card title="Inteligência de Recursos" icon={Activity}><EmptyState>Carregando métricas do escritório…</EmptyState></Card>;
  ...
  return (
    <Card title="Inteligência de Recursos" icon={Activity}>
      ...
      <div className="mt-3">
        ...
        {tab === 'teses' && (stats.melhoresTeses.length ? <ul className="space-y-2">{stats.melhoresTeses.map((t, i) => <TeseItem key={i} t={t} />)}</ul> : <EmptyState>Ainda não há teses de resultados favoráveis — conforme os tribunais julgam, as melhores teses aparecem aqui.</EmptyState>)}
