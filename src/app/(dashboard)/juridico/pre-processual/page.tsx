...
export default function PreProcessualPage() {
  ...
  return (
    <div className="flex h-full min-h-0 flex-1 flex-col bg-[#fafafa] dark:bg-zinc-950 text-[#101820] dark:text-zinc-200 max-lg:overflow-y-auto">
...
function Column({ phase, items, onOpen, onProtocolar, onChanged, canRename, onRename }: { phase: KanbanPhase; items: KanbanCard[]; onOpen: (id: string) => void; onProtocolar: (id: string) => void; onChanged: () => void; canRename: boolean; onRename: (key: string, label: string) => void }) {
  ...
  return (
    <div className="flex min-h-0 w-[280px] shrink-0 flex-col">
      ...
      <div ref={setNodeRef} className={`flex flex-col gap-2.5 rounded-xl border p-2.5 transition-colors lg:min-h-0 lg:flex-1 lg:overflow-y-auto ${isOver ? 'border-[#e11970] bg-[#e11970]/5' : 'border-[#dcdfe5] bg-[#f2f2f2] dark:border-zinc-800 dark:bg-zinc-900/40'}`}>
...
function Card({ c, onOpen, onProtocolar, onChanged }: { c: KanbanCard; onOpen?: (id: string) => void; onProtocolar?: (id: string) => void; onChanged?: () => void }) {
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes}
      ...
      className={`cursor-pointer touch-none rounded-lg border border-[#cfe0ed] bg-white py-3 pl-3 pr-3 shadow-sm transition-shadow hover:shadow-md active:cursor-grabbing dark:border-zinc-700 dark:bg-zinc-800 ${isDragging ? 'opacity-40' : ''}`}>
