 ...
 * do React Flow. Mesmo visual do nó do organograma (`agent-node.tsx`), mas
 * como um botão simples — clicar abre a edição; o pill de status alterna
 * ativo/pausado sem propagar o clique.
 */
export function AgentCard({
  ...
  return (
    <button
      ...
      className={`group flex w-full flex-col rounded-xl border bg-white p-4 text-left shadow-sm transition-shadow hover:shadow-md dark:bg-zinc-900 ${
        isOrchestrator
    ...
    >
      ...
      {agent.channels && agent.channels.length > 0 ? (
        ...
        <div className="mt-2">
          <span className="inline-flex items-center rounded-md bg-zinc-50 px-1.5 py-0.5 text-[10px] text-zinc-400 dark:bg-zinc-800/50 dark:text-zinc-500">
            Sem canal
          </span>
        </div>
      )}
