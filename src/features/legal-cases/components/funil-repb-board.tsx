...
// Board do FUNIL REPB (comercial / pré-contrato). Usado tanto como aba dentro da
// página REPB (toggle "Passivo | Funil de vendas", prop embedded) quanto na rota
...
export function FunilRepbBoard({ embedded = false }: { embedded?: boolean }) {
    ...
    <div className={rootClass}>
      <div className="shrink-0 border-b border-[#f0dcc9] dark:border-zinc-800 px-4 py-4 lg:px-6">
        <div className="flex flex-wrap items-center gap-x-2 gap-y-3">
          ...
          <div className="relative w-full sm:w-auto">
            ...
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar lead, CPF…"
      ...
      {view === 'lista' ? (
        ...
        <DndContext sensors={sensors} onDragStart={(e: DragStartEvent) => setActiveId(e.active.id as string)} onDragEnd={onDragEnd}>
          ...
          <KanbanBulkBar bulk={bulk} cards={filtered} phases={phases} queryKey={KEY} accent={ACCENT} />
        </DndContext>
      )}
...
function Card({ c, bulk, colIds, isNovos, terminal, onOpen, onFechou, onApresentar, onAgendar, onFollowup, staleFollowup }: { c: KanbanCard; bulk?: KanbanBulk; colIds?: string[]; isNovos?: boolean; terminal?: boolean; onOpen?: (id: string) => void; onFechou?: (c: KanbanCard) => void; onApresentar?: (c: KanbanCard) => void; onAgendar?: (c: KanbanCard) => void; onFollowup?: (c: KanbanCard) => void; staleFollowup?: boolean }) {
  const { attributes, listeners, setNodeRef, transform, isDragging } = useDraggable({ id: c.id });
  const clickedIds = useRepbSeenStore((s) => s.clickedIds);
  const markCardClicked = useRepbSeenStore((s) => s.markCardClicked);
  // Bolinha vermelha "novo lead" ao lado do card (só em Novos Leads), some ao clicar.
  const isNew = !!isNovos && !clickedIds.includes(c.id);
  ...
  return (
    <div ref={setNodeRef} style={style} {...listeners} {...attributes} {...cardAttr(c.id)}
      ...
      {isNew && <span title="Novo lead — clique para ver" className="absolute right-8 top-3 z-10 h-2.5 w-2.5 rounded-full bg-red-500 ring-2 ring-white dark:ring-[#1E2226]" />}
      ...
      <div className="mt-2 flex items-center justify-between gap-2 border-t border-[#eef2f8] pt-1.5 dark:border-zinc-800">
        <span className={`inline-flex items-center gap-0.5 text-[10px] font-semibold ${staleFollowup ? 'text-rose-600 dark:text-rose-400' : 'text-[#4b5863] dark:text-zinc-400'}`} title={staleFollowup ? 'Parado há dias — faça um follow-up' : 'Tempo na fase atual'}><Clock className={`h-3.5 w-3.5 ${staleFollowup ? 'text-rose-500' : 'text-[#ff6f00]'}`} /> {fmtDias(c.diasNaFase)}</span>
      ...
      {onFollowup && (
        <button
          onClick={(e) => { e.stopPropagation(); onFollowup(c); }}
          onPointerDown={(e) => e.stopPropagation()}
          title="Follow-up: roteiros de reengajamento + histórico"
          className="mt-2 inline-flex w-full items-center justify-center gap-1 rounded-lg border border-[#E8590C]/40 bg-[#E8590C]/5 px-2 py-1.5 text-xs font-semibold text-[#E8590C] hover:bg-[#E8590C]/10"
        >
