...
export const financeiroService = {
  ...
  async despesasDoCaso(caseId: string): Promise<{ txId: string; data: string; descricao: string; categoria: string; valor: number; pago: boolean; jaUsada: boolean }[]> {
    const { data } = await api.get('/financeiro/despesas-do-caso', { params: { caseId } });
    return data.data ?? data;
  },

  async importarExtratoLinhas(conta: string | null, linhas: { data: string; valor: number; descricao: string; area?: string | null; rateioVerticais?: { area: string; valor: number; label?: string }[]; contribuintes?: { userId?: string; nome: string; valor: number }[]; caseId?: string | null; contactId?: string | null; clienteNome?: string | null; pessoa?: { nome: string; userId?: string; categoria: string }; exito?: { bruto: number; cliente: number; sucumbencia: number; honorarios: number; valorCausa?: number; sucumbenciaPct?: number; honorariosPct?: number; sucumbenciaBase?: string; parcial?: boolean; totalExecutado?: number; anexos?: { name: string; mime: string; base64: string }[]; verbas?: { label: string; valor: number; natureza: 'proveito' | 'reembolso_cliente' | 'reembolso_escritorio' | 'sucumbencia_nossa' }[]; deducoesCliente?: { label: string; valor: number; tipo: 'sucumbencia_contraria' | 'despesa_reembolsavel' | 'outro'; cnjIncidente?: string; txIdSaida?: string }[]; grupoId?: string; beneficiarioAlvara?: 'cliente' | 'escritorio'; unificar?: boolean } | null; liquidarTxId?: string | null }[], saldoFinal?: number | null, saldoFinalEm?: string | null): Promise<{ importados: number; duplicados: number; baixados?: number; total: number; reconciliacao?: ReconConta | null }> {
    // saldoFinalEm = <DTASOF> do OFX: o INSTANTE a que o saldo se refere. Sem ele o back data a
    // âncora pela última linha do extrato — e o caixa desconta de novo o que já está no saldo.
    const { data } = await api.post('/financeiro/extrato/importar', { conta, linhas, commit: true, ...(saldoFinal != null && Number.isFinite(saldoFinal) ? { saldoFinal } : {}), ...(saldoFinal != null && saldoFinalEm ? { saldoFinalEm } : {}) }, { timeout: 180000 });
    return data.data ?? data;
  },
