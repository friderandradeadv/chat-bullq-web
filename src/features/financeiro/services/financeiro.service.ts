...
export const financeiroService = {
  ...
  async importarExtratoLinhas(conta: string | null, linhas: { data: string; valor: number; descricao: string; area?: string | null; rateioVerticais?: { area: string; valor: number; label?: string }[]; contribuintes?: { userId?: string; nome: string; valor: number }[]; caseId?: string | null; contactId?: string | null; clienteNome?: string | null; exito?: { bruto: number; cliente: number; sucumbencia: number; honorarios: number; valorCausa?: number; sucumbenciaPct?: number; honorariosPct?: number; sucumbenciaBase?: string; parcial?: boolean; totalExecutado?: number } | null; liquidarTxId?: string | null }[], saldoFinal?: number | null): Promise<{ importados: number; duplicados: number; baixados?: number; total: number; reconciliacao?: ReconConta | null }> {
